...
class WordGen
  def initialize(define_word)
    ...
    @word = @json[0]['word']
    @definition = Chooser.new(get_definitions)
  end

# grab json entry from api.dictionary.dev
def retrieve_json
  `curl https://api.dictionary.dev/api/v2/entries/en/#{@user_word}`
end

# get word if no word supplied
def query_user 
...
def clean_up_json(data)
  ...
  begin 
    ...
    puts "Problematic string (first 200 chars): #{safe_json_string[0..199]}..."
    exit
  end
  output
end
